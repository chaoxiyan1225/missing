...
public class MchUserServiceImpl extends GenericServiceImpl<TbMch> implements
	...
	//获取商家的微信关注用户
	@Override
	public List<TbWechatUser> getMchWechatUsers(
		...
		long allCount = mchUserDAO.getCountByQueryCondtion(TbWechatUser.class, query);
		...
		query.setOrderName("userRegisterTime");
		query.setAesc(false);

		try {

			return mchUserDAO.getListByQueryCondtion(TbWechatUser.class, query);
		} catch (DataAccessException e) {
	...
	public List<TransactionReportBean> generateOneWeekTransactionRecords(
		...
		for (int i = 0; i < 7; i++) {
			...
			long countFee = mchUserDAO.sumByQueryCondtion(
					TbTransactionRecord.class, query, "recordMoney")/100;
	...
	public List<TbSuggestion> getSuggestions(HttpServletRequest request,
		...
		int currentPage = ParseRequest.parseCurrentPage(request, model);
		Integer fkMchUserId = ParseRequest.getMchUserIdFromSession(request);
		List<TbSuggestion> suggestions = mchUserDAO.getListByProperty(TbSuggestion.class,
				"fkMchId", fkMchUserId);
		
		int tmpCnt = suggestions == null? 0:suggestions.size();
		int allPages = (tmpCnt
				+ PageConfigUtil.PAGE_COUNT_SIZE_10 - 1)
				/ PageConfigUtil.PAGE_COUNT_SIZE_10;
		model.put("allPages", allPages - 1);
