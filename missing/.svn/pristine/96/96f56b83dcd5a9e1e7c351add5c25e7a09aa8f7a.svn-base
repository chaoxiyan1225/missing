	...
	<header  class="bar"> 
	  <form name="search" class="search" id="search" action="">
	    <div class="search-row">
	      <input type="search" name="mchname" id="mchname"/>
	      <span class="placeholder "><span class="iconfont icon-sousuo"></span><span class="text"><font color="green">输入商家名搜索</font></span></span>
	    </div>
       </form>	
    </header>
	<div>
		<img src="/missing/staticfile/images/xianhua.jpg" style="width: 100%;" />
	</div>
	<br />
			...
			<c:forEach items="${mchUsers}" var="mchUser">
				<dl class="list" gaevent="common/poilist">
					<dd class="poi-list-item">
						<a class="react" 
						<c:if test="${type == 'queue'}">
	                         href="tosinglemch?mchId=${mchUser.mchUser.mchId}"
			            </c:if>
			            
			            <c:if test="${type == 'product'}">
	                         href="toproductsonemch?mchId=${mchUser.mchUser.mchId}"
			            </c:if>
								...
								<dd>
			                          ...
			                          <c:if test="${type == 'product'}">
										...
										<div class="dealcard dealcard-poi">
											<div
												...
												<img src="/missing/${mchUser.contentPictures[0]}"
													style="width: 100%;" />
											</div>
											<div class="dealcard-block-right">
												...
												<span
														class="tag"
														style="color: #fff; border-color: #E52010; background-color: #E52010;">进入商家</span>
												</div>
											</div>
										</div>
									</a>
									<p class="posi-right-bottom">
										<a class="statusInfo" onclick="return false;" href="#">已售<span
											class="strong">${mchUser.mchUser.totalSaleCount}</span></a>
									</p>
								</dd>
