        ...
        <div class="txtBox">
        	<c:choose>
        		<c:when test="${ empty  cacheTransactions}">
        			<div style="text-align:center; color:#ccc; font-size:32px; margin-top:200px;">亲爱的小二，您暂无订单</div>
        		</c:when>
        		<c:otherwise>
        			<c:forEach items="${cacheTransactions}" var="cacheTransaction">
        			 	<div class="oneTxt">
            				...
            				<c:if test="${cacheTransaction.transaction.recordStatus==2}">
		                       <button type="button" class="yongyin" style="width:100px; height:30px;" onclick="window.location.href='tofinishtransaction?recordId=${cacheTransaction.transaction.recordId}'" >
            				   <font>点我完成</font></button>
		                    </c:if>
		                    <br />
               			 <span>
...
function showMessage(message,type,time) {
 ...
 var tofinish = function(id){
	...
	window.location.href="<%=request.getContextPath()%>/wechatuser/tofinishtransaction?recordId="+id;
