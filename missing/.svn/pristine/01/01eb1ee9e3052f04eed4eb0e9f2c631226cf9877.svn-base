...
public class ClientTyeUtil {
	
	/**
	 * 根据http的useragent参数确定是否是移动客户端
	 * @param userAgent
	 * @return
	 */
	public boolean isMoblieClient(String userAgent) {
		...
		if(StringUtils.isNotBlank(userAgent)){
 			for (String mobileAgent : mobileAgents) {
 				if (userAgent.toLowerCase().indexOf(mobileAgent) >= 0 &&userAgent.toLowerCase().toLowerCase().indexOf("macintosh") <= 0) {
 					isMoblie = true;
 					break;
 				}
 			}
