...
<script type="text/javascript">
	...
	function showMessage(message,type,time) {
	 ...
	 var cancelorder = function(id){
		if(id.length == 0)
	    {
			showMessage("必须选择一个排号码",'error',1000);
	    	return false;
	    }

		$.post("<%=request.getContextPath()%>/wechatuser/orderchange",{orderid:id,status:'CACELED'},function(data){
			 window.current.href="<%=request.getContextPath()%>/mchuser/toqueue";
	    });
...
<div class="page">
    ...
    <div style="position:relative; width:100%; background:#131313; position:relative; z-index:10">
        <div class="txtBox">
        	<c:choose>
        		<c:when test="${ empty  queueInfos}">
        			<div style="text-align:center; color:#ccc; font-size:32px; margin-top:200px;">亲爱的大人，您还未取号</div>
        		</c:when>
        		<c:otherwise>
        		   <div class="oneTxt">
        		       客官您有如下排号信息：
        		       ...
        		       <span>-------------------------------------------------------------</span>
        		       <br/>
        			<c:forEach items="${queueInfos}" var="queueInfo">
        			 	<span >
            				排号码：${queueInfo.record.orderId} | 商家：${queueInfo.mchShopName}<button type="button" class="yongyin" onclick="cancelorder(${queueInfo.record.recordId})" style="width:100px; height:28px;">取消排号</button>
               			 </span>
               			 <br />
               			 <span>
                                                                                      前面等待人数：${queueInfo.numFront}个 |
	               		  </span>
               			 <span>
                                                                                      大约需要等待：${queueInfo.waitMins}分钟 |
	               		  </span>
	               		  <span>
	               		  取号时间：
	               		  <fmt:formatDate value="${queueInfo.record.startTime}" pattern="yyyy-MM-dd HH:mm"/>
	               		  </span>
	               		  <br />
	               		  <span>-------------------------------------------------------------</span>
	               		  <br />
